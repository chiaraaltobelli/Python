...
class Popup(EasyDialog):
    def __init__ (self, parents):
        EasyDialog.__init__(self,parents, "Warning")

    def body(self, master):
        self.addLabel(master, text = "You have reached the limit of three failed login attempts. The police have been called.",\
            row = 0, column = 0)

#Class to notify user that max login attempts (3) have been reached and police have been called
class ATM(EasyFrame):
    ...
    counter = 0 #count number of login attempts
    ...
    def login(self):
        ...
        if self.account:
            ...
            self.counter = self.counter + 1 #increase counter to track number of login attempts before calling police
            if self.counter == 3:
                self.loginButton["state"] = "disabled" #disable login button
                popup = Popup(self) #popup with warning message
                popup.mainloop()
